...
import { fetchStoredProviderUsage, parseProviderCredentials, providerRecordsToUsageEvents } from "./provider-runtime";
...
describe("provider runtime", () => {
  ...
  it("동일 사용량 레코드는 조회를 반복해도 같은 이벤트 ID를 만든다", () => {
    ...
    expect(first).toEqual(expect.objectContaining({ source: "provider_api", projectId: "project-1", inputTokens: 10, outputTokens: 3 }));
  });
